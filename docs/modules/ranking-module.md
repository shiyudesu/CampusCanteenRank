...
15. 排行缓存失效策略已从“前缀扫描批量删除”升级为“版本号失效（version bump）”，写路径仅递增版本键，避免高写入场景 `SCAN + DEL` 放大。
16. 排行缓存回源已加入 Redis 分布式锁（`SET NX + EX` + Lua compare-and-delete 解锁），缓存未命中时仅允许单请求回源构建，其他请求短暂等待缓存填充，降低击穿风险。

### 4.2 下一步计划（Next Steps）

1. 增加游标浮点边界测试（同分值/高并发分页）并验证跨页稳定性。
2. 为排行榜缓存重建链路补充压力测试基线（锁争用、回源时延、命中率），输出可观测指标阈值。
...
2. 当前缓存采用版本号失效与分布式锁防击穿，需持续关注锁 TTL 与等待重试参数在高延迟网络下的取值合理性。
